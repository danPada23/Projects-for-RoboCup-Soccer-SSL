classdef Field < handle
    ...
    methods
        ...
        function resolve_collision(obj, bot, ball, fsm_state, team_id)
            % Se lo stato non viene passato (retrocompatibilità), assumiamo -1
            if nargin < 4
                fsm_state = -1;
            end
            % Se il team_id non viene passato, assegniamo un tocco neutro
            if nargin < 5
                team_id = 'none';
            end
            ...
            if dist_B <= (b_rad + ball.r_p) && fsm_state ~= 3 && fsm_state ~= 7
                r_r = [bot.b * cos(bot.theta); bot.b * sin(bot.theta)]; 
                n_vec = [ball.x; ball.y] - B_center;
                n_hat = n_vec / norm(n_vec);                            
                
                v_B = [v_r_vec(1) - bot.omega * r_r(2);
                       v_r_vec(2) + bot.omega * r_r(1)];                
                
                v_rel_n = dot(v_B - [ball.vx; ball.vy], n_hat);         
                if v_rel_n > 0
                    cross_rn = r_r(1)*n_hat(2) - r_r(2)*n_hat(1);       
                    num = (1 + bot.e) * v_rel_n;
                    ...
                    % --- REGISTRAZIONE ULTIMO TOCCO ---
                    ball.last_touch = team_id; 
                end
            elseif dist_C <= (c_rad + ball.r_p)
                n_vec = [ball.x; ball.y] - C_center;
                n_hat = n_vec / norm(n_vec); 
                v_rel_n = dot(v_r_vec - [ball.vx; ball.vy], n_hat);  
