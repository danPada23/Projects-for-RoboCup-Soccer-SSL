...
% --- CARICAMENTO AGENTI RL (Scontro Epico) ---
% Carichiamo le reti in due variabili diverse per evitare sovrascritture
data_L = load('Agente_Standard_nuovo.mat'); 
agent_L = data_L.agent;

data_R = load('Agente_Standard.mat'); % Metti qui lo sfidante!
agent_R = data_R.agent;
...
while ishandle(Scenario)
    ...
    % Il Planner Sinistro esegue la cinematica in base allo stato scelto dall'IA
    [u1_L, u2_L] = planner_left.decide_action(bot_left, bot_right, palla, campo, X_max, Y_max, d);
    
    % --- 1B. L'IA DECIDE LA TATTICA PER IL BOT DESTRO (CON SPECCHIO) ---
    if planner_right.fsm_state == 0
        
        % 1. CREAZIONE COORDINATE VIRTUALI (Ribaltamento asse X)
        pos_R_virt = [X_max - bot_right.x, bot_right.y];
        pos_L_virt = [X_max - bot_left.x, bot_left.y];
        pos_P_virt = [X_max - palla.x, palla.y];
        theta_R_virt = pi - bot_right.theta; % Specchio l'angolo di imbardata
        
        % 2. CALCOLO OSSERVAZIONE (Identico al bot L, ma coi dati virtuali)
        d1_R = norm(pos_P_virt - pos_R_virt) / diag_campo;
        
        a2_R = atan2(pos_P_virt(2) - pos_R_virt(2), pos_P_virt(1) - pos_R_virt(1)) - theta_R_virt;
        a2_R = atan2(sin(a2_R), cos(a2_R)) / pi;
        
        d3_R = norm(pos_L_virt - pos_R_virt) / diag_campo;
        
        a4_R = atan2(pos_L_virt(2) - pos_R_virt(2), pos_L_virt(1) - pos_R_virt(1)) - theta_R_virt;
        a4_R = atan2(sin(a4_R), cos(a4_R)) / pi;
        
        d5_R = norm([0, Y_max/2] - pos_R_virt) / X_max;         % Distanza alla porta virtuale
        d6_R = norm([X_max, Y_max/2] - pos_R_virt) / X_max;     % Distanza porta avversaria virtuale
        
        % Vantaggio = dist(avversario, palla) - dist(mia, palla)
        v7_R = (norm(pos_P_virt - pos_L_virt) - norm(pos_P_virt - pos_R_virt)) / diag_campo; 
        
        obs_R = [d1_R; a2_R; d3_R; a4_R; d5_R; d6_R; v7_R];
        
        % 3. CHIAMATA ALLA RETE NEURALE "MANCINA"
        act_R = getAction(agent_R, {obs_R});
        action_idx_R = act_R{1};
        
        % 4. MAPPATURA AZIONI
        switch action_idx_R
            case 1, planner_right.fsm_state = 1; % PURSUE
            case 2, planner_right.fsm_state = 2; % BACK
            case 3, planner_right.fsm_state = 4; % CUSTOM
            case 4, planner_right.fsm_state = 3; % DIFESA
        end
    end
    
    % Il Planner Destro esegue la cinematica (le coordinate fisiche reali le gestisce lui!)
    [u1_R, u2_R] = planner_right.decide_action(bot_right, bot_left, palla, campo, X_max, Y_max, d);

    % =========================================================
    % --- OVERRIDE VETTORIALE DI FUGA (Bypassa il Planner) ---
    % =========================================================
    velocita_fuga = 0.7; % Velocità fluida in m/s per aggirare la palla
    
    % --- 1. OVERRIDE BOT SINISTRO ---
    dist_L_P = norm([palla.x - bot_left.x, palla.y - bot_left.y]);
    
    % Si attiva SOLO se il bot non è in stato STOP (7)
    if dist_L_P <= R_mid && bot_left.x > palla.x - 0.02 && planner_left.fsm_state ~= 7
        
        target_x = palla.x - 0.15;
        if bot_left.y >= palla.y
            target_y = palla.y + 0.15; 
        else
            target_y = palla.y - 0.15; 
        end
        target_y = max(campo.safe_y(1), min(campo.safe_y(2), target_y));
        
        dir_x = target_x - bot_left.x;
        dir_y = target_y - bot_left.y;
        mag = norm([dir_x, dir_y]);
        
        if mag > 0.01
            u1_L = (dir_x / mag) * velocita_fuga;
            u2_L = (dir_y / mag) * velocita_fuga;
        end
    end

    % --- 2. OVERRIDE BOT DESTRO (Speculare) ---
    dist_R_P = norm([palla.x - bot_right.x, palla.y - bot_right.y]);
    
    % Si attiva SOLO se il bot non è in stato STOP (7)
    if dist_R_P <= R_mid && bot_right.x < palla.x + 0.02 && planner_right.fsm_state ~= 7
        
        target_x = palla.x + 0.15;
        if bot_right.y >= palla.y
            target_y = palla.y + 0.15; 
        else
            target_y = palla.y - 0.15; 
        end
        ...
        dir_x = target_x - bot_right.x;
        dir_y = target_y - bot_right.y;
        mag = norm([dir_x, dir_y]);
        
        if mag > 0.01
            u1_R = (dir_x / mag) * velocita_fuga;
            u2_R = (dir_y / mag) * velocita_fuga;
        end
    end
    % =========================================================
    
   if planner_left.fsm_state == 7
        bot_left.err_sum_x = 0;
        bot_left.err_sum_y = 0;
        u1_L = 0; u2_L = 0; % Sicurezza extra
    end
    
    if planner_right.fsm_state == 7
        bot_right.err_sum_x = 0;
        bot_right.err_sum_y = 0;
        u1_R = 0; u2_R = 0; % Sicurezza extra
    end
    ...
    if goal_status > 0
        ...
        fprintf('--- PUNTEGGIO ATTUALE: RL %d  |  RL %d ---\n\n', score_L, score_R);
   ...
   % Calcolo minuti e secondi per il tabellone a schermo
    ...
    title(sprintf('Partita 1vs1 | Tempo %d - %02d:%02d | Punteggio: RL %d - %d RL', ...
